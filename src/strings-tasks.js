...
function removeFirstOccurrences(str, value) {
  ...
  if (ind === -1) return str;
  const lastInd = ind + value.length;
  const res = str.substring(ind, lastInd);
...
function removeLastOccurrences(str, value) {
  if (!str) return '';
  const ind = str.toLowerCase().lastIndexOf(value.toLowerCase());
  if (ind === -1) return str;
  const lastInd = ind + value.length;
  const newStr = str.slice(0, ind) + str.slice(lastInd);
  return newStr;
}
...
function formatTime(min, sec) {
  let strMin;
  let strSec;
  if (min < 10) strMin = min.toString().padStart(2, '0');
  if (min >= 10) strMin = min.toString();
  if (sec < 10) strSec = sec.toString().padStart(3, ':0');
  if (sec >= 10) strSec = sec.toString().padStart(3, ':');
  return strSec.padStart(5, strMin);
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    for (let j = 0; j < vowels.length; j += 1) {
      if (str.charAt(i) === vowels[j]) count += 1;
    }
  }
  return count;
}
...
function findLongestWord(sentence) {
  ...
  let maxString = arr[0];
  arr.forEach((el) => {
    maxString = el.length > maxString.length ? el : maxString;
  });
  return maxString;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    curr.push(arr[i].split('').reverse().join(''));
  }
  return curr.join(' ');
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const sign = str[i];
    if (str[i] === sign.toLowerCase()) {
      newStr += sign.toUpperCase();
    } else {
      newStr += sign.toLowerCase();
    }
  }
  return newStr;
}
...
function encodeToRot13(str) {
  ...
  const str2 = 'nopqrstuvwxyzabcdefghijklm';
  let newStr = '';
  for (let i = 0; i < str.length; i += 1) {
    // reg Exp (/[^\w]/) - любой символ кроме букв
    if (str[i].match(/[^\w]/)) {
      newStr += str[i];
    }
    for (let j = 0; j < str1.length; j += 1) {
      if (str[i].toLowerCase() === str1[j]) {
        if (str[i].toLowerCase() === str[i]) {
          newStr += str2[j];
        } else newStr += str2[j].toUpperCase();
      }
    }
  }
  return newStr;
...
function getCardId(value) {
  const suits = ['♣', '♦', '♥', '♠'];
  let index;
  const i = suits.indexOf(value[value.length - 1]);
  if (value.includes(suits[i])) {
    index = parseInt(value, 10) - 1 + 13 * i;
  }
  if (value.includes('A')) {
    index = 13 * i;
  }
